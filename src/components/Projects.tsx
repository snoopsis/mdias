...
const projectMeta = [
  ...
  { icon: Globe, links: { github: "https://github.com/snoopsis/cpmve" }, featured: false, tech: ["HTML", "CSS", "JavaScript"] },
  { icon: Code2, links: { github: "https://github.com/snoopsis/phpMVC" }, featured: false, tech: ["PHP", "MVC", "MySQL", "Architecture"] },
];
  ...
  return (
    <section id="projects" className="py-24 px-6 border-t border-card-border">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <p className="text-sm text-accent font-mono mb-2">{t.projects.label}</p>
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight">
            {t.projects.title}
          </h2>
          <p className="mt-3 text-muted max-w-lg">{t.projects.subtitle}</p>
        </motion.div>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex gap-3">
                    {project.links.live && (
                      <a href={project.links.live} target="_blank" rel="noopener noreferrer" className="text-muted hover:text-accent transition-colors" aria-label="Live site">
                        <ExternalLink size={16} />
                      </a>
                    )}
                    {project.links.github && (
                      <a href={project.links.github} target="_blank" rel="noopener noreferrer" className="text-muted hover:text-accent transition-colors" aria-label="GitHub">
                        <Github size={16} />
                      </a>
                    )}
                  </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tech.map((tech) => (
                    <span key={tech} className="text-xs font-mono px-2 py-1 rounded-md bg-accent/5 text-accent/80 border border-accent/10">{tech}</span>
                  ))}
                </div>
              </motion.div>
            ))}
          ...
          {projects
            ...
            .map((project, i) => (
              ...
              >
                ...
                <h3 className="text-sm font-semibold mb-1.5 group-hover:text-accent transition-colors">{project.title}</h3>
                <p className="text-xs text-muted leading-relaxed mb-3">{project.description}</p>
                <div className="flex flex-wrap gap-1.5">
                  {project.tech.slice(0, 3).map((tech) => (
                    <span key={tech} className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-card-border/50 text-muted">{tech}</span>
                  ))}
                </div>
              </motion.div>
            ))}
        ...
        >
          <a href="https://github.com/snoopsis" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-muted hover:text-accent transition-colors">
            <Github size={16} />
            {t.projects.github}
          </a>
        </motion.div>
